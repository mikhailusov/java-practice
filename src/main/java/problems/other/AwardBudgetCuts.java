 ...
 * due to spending cutbacks, the budget was reduced to newBudget dollars and now they need to reallocate the grants.
 * The committee made a decision that they’d like to impact as few grant recipients as possible by applying a maximum
 * cap on all grants. Every grant initially planned to be higher than cap will now be exactly cap dollars. Grants less
    ...
    public static double findGrantsCap(double[] grantsArray, double newBudget) {
        ...
        for (int i = 0; i < grantsArray.length; i++) {
            if (grantsArray[i] < cap) {
                ...
                cap = newBudget / (grantsArray.length - (i + 1));
            } else {
                break;
            }
        }
